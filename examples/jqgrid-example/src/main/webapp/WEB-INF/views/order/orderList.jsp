...
$("#${uuid}").jqGrid({        
   	url:"${ctx}/oa/order/search",
	datatype: "json",	
   	colNames:['ID','名称','总数量','总金额','创建时间','创建人','当前节点','当前处理人/角色','流程定定义ID','流程定定义KEY','流程实例ID','任务JSON','当前处理任务信息'],
   	colModel:[
   	    {name: "id", index:'id',width:0,key:true,hidden:true}, 
   	 	{name:'name',index:'name', width:150,editable:true,editrules:{required:true} },
   	 	{name:'quantity',index:'quantity', width:100,editable:true,editrules:{required:true,integer:true}},
	...
	caption: "订单列表",
	autowidth:true,
	editurl: "${ctx}/oa/order/generic/edit",	
	
	onSelectRow : function(rowid,status,e){		
		$.workflow.decision({gridId:"${uuid}",businessKey:rowid,processDefinitionKey:'leave-dynamic-from',start:false,
			buttons:[
...
$("#${uuid}").jqGrid('navGrid','#p${uuid}',
	...
	{},// add options
	{
		url:'${ctx}/oa/order/generic/delete',
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{
	...
	onClickButton:function(){								
		$.workflow.dynamic.showStartupProcessDialog({gridId:"${uuid}",path:'oa/order',processDefinitionKey:'leave-dynamic-from',
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{
	...
	buttonicon : "ui-icon-check",
	 position : "last",
	onClickButton:function(){		
		$.workflow.task.claim({gridId:'${uuid}',path:'oa/order',
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{
	...
	buttonicon : "ui-icon-close",
	 position : "last",
	onClickButton:function(){		
		$.workflow.task.unclaim({gridId:'${uuid}',path:'oa/order',
			buttons:[{name:'start_${uuid}',show:false},
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{
	...
	buttonicon : "ui-icon-check",
	 position : "last",
	onClickButton:function(){
		$.workflow.dynamic.showTaskDialog({gridId:"${uuid}",path:'oa/order',dialogopts:{width:400,height:300},
			buttons:[{name:'start_${uuid}',show:false},
			         {name:'complete_${uuid}',show:false},
			         {name:'unclaim_${uuid}',show:false},
			         {name:'delegate_${uuid}',show:false},
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{
	...
	position : "last",
	onClickButton:function(){		
		$.workflow.task.delegate({gridId:'${uuid}',path:'oa/order',
			buttons:[{name:'start_${uuid}',show:false},
			         {name:'complete_${uuid}',show:false},
			         {name:'unclaim_${uuid}',show:false},
			         {name:'delegate_${uuid}',show:false},
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{
	...
	buttonicon : "ui-icon-close",
	 position : "last",
	onClickButton:function(){		
		$.workflow.task.resolve({gridId:'${uuid}',path:'oa/order',
			buttons:[{name:'start_${uuid}',show:false},
			         {name:'complete_${uuid}',show:true},
			         {name:'unclaim_${uuid}',show:false},
			         {name:'delegate_${uuid}',show:true},
...
jQuery("#${uuid}").jqGrid('navButtonAdd','#p${uuid}',{	
	...
	buttonicon : "ui-icon-star",
	 position : "last",
	onClickButton:function(){		
		$.workflow.historic.view("${uuid}",{processDefinitionKey:'leave-dynamic-from',actionVariableNames:'deptLeaderPass_text,hrPass_text,reApply_text',reasonVariableNames:'hrBackReason,leaderBackReason'});		
	} 
});
