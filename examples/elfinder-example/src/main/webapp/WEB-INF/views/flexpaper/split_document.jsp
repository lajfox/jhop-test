...
<body>
	...
	<script type="text/javascript">  
		...
		function getNumPages(){		
			...
			var result = $.ajax({
                url:"${ctx}/flexPaper/office2pdf?target=${param['target']}",
                dataType:"json",               
                async:false,
                cache:false
            }).responseText;			
		...
		function getDocumentUrl(document){
			...
			var numPages = parseInt(getNumPages(),10);
			</c:when>
			<c:otherwise>
			...
			var url = "{${ctx}/flexPaper/view?target=${param['target']}&doc={doc}&format={format}&page=[*,0],{numPages}}";
		...
		var searchServiceUrl = escape("${ctx}/flexPaper/view?target=${param['target']}&doc=&format=swfstrings&page=[page]&searchterm=[searchterm]");
