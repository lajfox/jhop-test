...
public final class MyJpaQueryLookupStrategy {
	...
	private static class DeclaredQueryLookupStrategy extends AbstractQueryLookupStrategy {
		...
		protected RepositoryQuery resolveQuery(JpaQueryMethod method, EntityManager em, NamedQueries namedQueries) {
			...
			RepositoryQuery query = MySimpleJpaQuery.fromQueryAnnotation(method, em);
			...
			if (namedQueries.hasQuery(name)) {
				return new MySimpleJpaQuery(method, em, namedQueries.getQuery(name));
			}

			query = MyNamedQuery.lookupFrom(method, em);
