...
public class ExportController {
	...
	public HttpEntity<byte[]> exporter(@RequestParam(value = "svg", required = false) String svg,
			...
			@RequestParam(value = "width", required = false) String width, @RequestParam(value = "scale", required = false) String scale,
			@RequestParam(value = "options", required = false) String options,
			@RequestParam(value = "constr", required = false) String constructor,
			@RequestParam(value = "callback", required = false) String callback,
			@RequestParam(value = "async", required = false, defaultValue = "false") Boolean async, HttpServletRequest request,
			HttpSession session) throws ServletException, InterruptedException, SVGConverterException, NoSuchElementException,
		...
		/*
		 * If tempFilename is not null, then we want to save the filename in
	...
	public void getFile(@PathVariable("name") String name, @PathVariable("ext") String ext, HttpServletResponse response)
	...
	public ModelAndView handleSVGRasterizeException(Exception ex, HttpServletResponse response) {
		ModelAndView modelAndView = new ModelAndView();
		modelAndView.setViewName("highcharts/error");
		modelAndView.addObject("message", "Something went wrong while converting. " + ex.getMessage());
		response.setStatus(500);
		return modelAndView;
	}
	...
	private String processRequest(String svg, MimeType mime, String width, String scale, String options, String constructor,
			String callback, String filename) throws SVGConverterException, PoolException, NoSuchElementException, TimeoutException,
			ServletException {
